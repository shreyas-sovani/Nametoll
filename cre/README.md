# Nametoll Brain (CRE Confidential Workflow)

Scaffold: official `hello-confidential-workflows-ts` from `cre templates list` / `smartcontractkit/cre-templates`. `cre init` needs a logged-in CLI (`cre login`); this tree is that template, then customized.

Nametoll customization (B9):
...
```bash
cd nametoll-brain && bun install && bunx cre-setup && cd ..
cp .env.example .env   # set SPEND_CAP_TINYBARS_VAR; never commit .env
