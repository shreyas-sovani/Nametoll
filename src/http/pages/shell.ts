...
const FAVICON = `data:image/svg+xml,${encodeURIComponent(
  `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 32 32"><rect width="32" height="32" rx="7" fill="#0b0d11"/><circle cx="16" cy="16" r="8.5" fill="none" stroke="#ffb224" stroke-width="2.5"/><circle cx="16" cy="16" r="3.4" fill="#ffb224"/></svg>`,
)}`;
...
:root {
  ...
  --amber: #ffb224;
  --amber-hi: #ffc95e;
  --amber-dim: rgba(255,178,36,0.13);
  --green: #3ddc97;
  --green-dim: rgba(61,220,151,0.12);
  --red: #ff6b57;
  --red-dim: rgba(255,107,87,0.1);
...
body::before {
  ...
  z-index: -2;
  background:
    radial-gradient(900px 420px at 18% -12%, rgba(255,178,36,0.07), transparent 60%),
    radial-gradient(700px 500px at 95% 8%, rgba(61,220,151,0.04), transparent 55%);
  pointer-events: none;
}
body::after {
  content: "";
  ...
  inset: 0;
  z-index: -1;
  background-image: repeating-linear-gradient(90deg, rgba(255,255,255,0.022) 0 1px, transparent 1px 72px);
  -webkit-mask-image: radial-gradient(1200px 600px at 50% 0, #000 0%, transparent 75%);
  mask-image: radial-gradient(1200px 600px at 50% 0, #000 0%, transparent 75%);
  pointer-events: none;
}
a { color: var(--text); }
::selection { background: rgba(255,178,36,0.3); }
code, kbd { font-family: var(--mono); font-size: 0.85em; color: var(--amber-hi); }
.skip {
  position: absolute;
  left: 0.75rem;
  top: -3rem;
...
.nav-cta:hover {
  ...
  box-shadow: 0 6px 22px rgba(255,178,36,0.35);
}
.wrap {
  width: min(76rem, calc(100% - 2rem));
  margin: 0 auto;
...
.lamp[data-state="deny"], .lamp[data-state="error"] {
  background: var(--red);
  box-shadow: 0 0 12px rgba(255,107,87,0.6);
}
.lamp[data-state="busy"], .lamp[data-state="unpaid"], .lamp.hero-lamp {
  background: var(--amber);
  box-shadow: 0 0 12px rgba(255,178,36,0.7);
}
...
button:hover, .btn:hover { transform: translateY(-1px); background: #262c37; }
button:disabled { opacity: 0.4; cursor: not-allowed; transform: none; }
button.pay {
  background: var(--amber);
  color: #241a04;
}
button.pay:hover { box-shadow: 0 6px 22px rgba(255,178,36,0.4); background: var(--amber-hi); }
button.ghost, .btn.ghost {
  background: transparent;
  color: var(--text);
  box-shadow: inset 0 0 0 1px var(--line-2);
}
button.ghost:hover, .btn.ghost:hover { box-shadow: inset 0 0 0 1px var(--amber); color: var(--amber-hi); background: transparent; }
...
h1 em, .it { font-style: italic; color: var(--amber-hi); }
h2 {
  font-family: var(--display);
  font-weight: 400;
  font-size: 1.85rem;
...
.btn-solid:hover {
  ...
  box-shadow: 0 8px 28px rgba(255,178,36,0.38);
}
.hero-card {
  position: relative;
...
.hero-card .stamp {
  ...
  background: var(--ink-0);
  box-shadow: 0 6px 18px rgba(255,178,36,0.18);
}
...
.flow::after {
  ...
  border-radius: 50%;
  background: var(--amber);
  box-shadow: 0 0 14px rgba(255,178,36,0.9);
...
.flow-step .v { margin-left: auto; font-family: var(--mono); font-size: 0.78rem; color: var(--mute); }
@keyframes step-glow {
  0%, 14%, 100% { color: inherit; }
  18%, 30% { color: var(--amber-hi); }
}
...
.marquee-group b { color: var(--mute); font-weight: 500; }
.marquee-group span::after { content: "·"; margin-left: 2.5rem; color: var(--line-2); }
...
.status-grid dt { color: var(--mute-2); }
.status-grid dd { margin: 0; word-break: break-all; color: var(--text); }
.status-grid dd a { color: var(--amber-hi); text-decoration: none; border-bottom: 1px dotted var(--amber); }
...
.rail-card { padding: 1.2rem 1.25rem; }
...
.banner.err, .banner.deny { border-left-color: var(--red); color: var(--red); background: var(--red-dim); }
.banner.ok { border-left-color: var(--green); color: var(--green); background: var(--green-dim); }
.stations {
  display: grid;
...
.doc-toc a:hover { color: var(--amber-hi); border-left-color: var(--amber); }
.doc-card { padding: 1.4rem 1.55rem 1.6rem; margin: 0 0 1.1rem; }
.doc-card h2 { scroll-margin-top: 4.5rem; }
.doc-card p, .doc-card li { color: var(--mute); }
.doc-card a { color: var(--amber-hi); }
.doc-card pre {
  overflow: auto;
  background: var(--ink-0);
  border: 1px solid var(--line);
...
.doc-card th { color: var(--mute-2); font-weight: 500; font-family: var(--mono); font-size: 0.78rem; letter-spacing: 0.06em; text-transform: uppercase; }

@media (max-width: 900px) {
  .hero { grid-template-columns: 1fr; gap: 2.2rem; }
  .top-inner { flex-wrap: wrap; padding: 0.7rem 0; }
  .lane, .band-grid, .catalog, .desk, .docs, form.drive, .stations {
...
<head>
  ...
  <link href="https://fonts.googleapis.com/css2?family=Instrument+Sans:wght@400;500;600&amp;family=Instrument+Serif:ital@0;1&amp;family=Spline+Sans+Mono:wght@400;500;600&amp;display=swap" rel="stylesheet" />
