...
| **Directory · session** | `POST /desk/session` creates an in-memory ECDSA guest buyer and faucets 0.05 HBAR from the seller. Cookie `nametoll_guest`. Rate-limited with pay. Faucet returns 503 if seller HBAR is under 5 HBAR or 16 guests are already live. | Hedera `AccountCreate` + `TransferTransaction` | Return the private key |
...
| **Merchandise** | Live Aave v3 + Compound III snapshot; billable units = delivered protocols. `GET /desk/risk?wallet=` is a 1-unit desk-side health factor (wallet is not a TEE input) | Studio gateway, pinned subgraph ids | Be a Graph prize SKILL; claim privacy of public positions |
...
TEE verdicts are **cached per amount + payer + hour-count** (`GET /health` → `brain.verdictTtlMs`, default 60s, set `VERDICT_TTL_MS=1800000` on judging day). The process re-warms the 1-unit and 2-unit amounts every half-TTL. One `cre workflow simulate` runs at a time; a hung child is killed at 60s and the desk fails closed. Unavailable / simulate failures are not cached. The enclave still gates every amount the first time it is seen in that TTL. Policy secrets (cap, optional `BUYER_ALLOWLIST`, optional `RATE_LIMIT`) stay in the TEE; public reasons are `under cap` / `over cap` / `buyer not allowlisted` / `rate limited`. A successful HCS bill may carry `verdictReason` and `verdictHash` of that public verdict. Recompute is still `units * priceTinybarsPerUnit = tinybars`.
...
- **Demo hardening** Brain caches successful verdicts per amount + payer + hour-count (TTL env-tunable; keep-warm every TTL/2; simulate serialized and killed at 60s). `POST /desk/pay` is rate-limited, optionally `DESK_PAY_SECRET`, and pinned to `PUBLIC_DESK_URL`. Pay omits the HCS bill block unless `settleTx` matches. Tagline is metered units.
- **B27** TEE keep-warm + serialize + 60s kill. `VERDICT_TTL_MS` (default 60s).
- **B28** Guest faucet 0.05 HBAR, 5 HBAR seller floor → HTTP 503, max 16 guests, seller balance on `/health`.
- **B29** Risk-score SKU: `GET /desk/risk?wallet=` behind the same 402 at 1 unit. Desk-side HF from pinned Messari markets. HCS `sku: risk-score`. Wallet is not a TEE input.
