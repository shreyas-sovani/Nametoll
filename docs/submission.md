...
- [x] **HTS or custom fees / Scheduled Transactions (extra points)** — Blocky402 `GET /supported` lists Hedera `exact` with no asset list, so `/desk/snapshot` stays HBAR `0.0.0`. Live TOLL `0.0.10483302` (custom fixed `100000` tinybar HBAR fee) https://hashscan.io/testnet/token/0.0.10483302. Two `wait_for_expiry` slots executed: https://hashscan.io/testnet/tx/0.0.10463842@1789156008.769559934 and https://hashscan.io/testnet/tx/0.0.10463842@1789156009.185023222 (1 TOLL + 100000 tinybars each). `GET /desk/claim` delivered live Aave and wrote `subscribe` bills on topic `0.0.10464309`. ERC-8004 / HCS-14 left out.
