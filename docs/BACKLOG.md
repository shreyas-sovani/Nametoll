...
- [x] **B17** Discovery directory. `/desks` + `GET /desk/catalog?parent=` enumerate Omnigraph `subdomains(first: 20)` when ENSNode answers; on TLS/`fetch failed` or ENSNode 4xx/5xx, walk `getSubregistry` + official `LabelRegistered` on the parent UserRegistry (3×8k recent blocks). Resolve each child, probe `/desk/offer` or unpaid 402. `npm run agent -- <parent>` picks a live desk by price/protocols and pays via `payFromName`. Hedera extra-points “directory that makes your service findable by other agents.” 12 Sep 2026.
