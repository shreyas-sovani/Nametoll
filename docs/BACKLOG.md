...
**Status 11 Sep 2026:** B0–B15 done. Live parent is `nametoll.eth` (child `desk.nametoll.eth`) on ENSv2 Sepolia. CRE simulate logs in `docs/partners/chainlink/`. Judge blotter is `/`. Submission timestamps + §9 checklists: `docs/submission.md`. Unused remainder refunds when delivered units < prepaid (Pinout shape, no dual HCS). Harness PR: https://github.com/hedera-dev/hedera-harness/pull/59 (open against `dev`; follow-up commit neutralized Scaffold-HBAR static checks, persisted `constraints.packageManager`, and rewrites only files `init` just wrote). Same CRE engine emits unsigned `join()` to live ChallengeLending `0x88574e7Cc0027afd04951daa09B64d4441931ba1`. No join tx broadcast. Next stretch is B16 only if you still want a Sunday form swap.
...
| **Next** | Stretch **B16** Sunday form swap. None of these are required for the spine. |
