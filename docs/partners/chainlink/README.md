...
Desk Brain **caches successful verdicts per amount + payer + hour-count** (`GET /health` → `brain.verdictTtlMs`, default 60s; set `VERDICT_TTL_MS=1800000` for judging day). A cold `cre workflow simulate` is ~9s; repeats of the same key reuse that attested result until TTL. The process re-warms 1-unit and 2-unit amounts every half-TTL. One simulate runs at a time. Failures (`TEE unavailable`, simulate timeout after 60s, not-logged-in) are **not** cached. A judge asking “does the enclave see every request?”: no — it sees every **amount/payer/hour** in the TTL. The wallet on `/desk/risk` is not a TEE input. Deny/allow still comes from the TEE.
